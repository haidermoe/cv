...
# INJECT TORNADO REST API HANDLERS DIRECTLY INTO STREAMLIT SERVER PROCESS
try:
    ...
    if server and hasattr(server, "_tornado_app") and server._tornado_app:
        ...
        class PythonHeadersHandler(tornado.web.RequestHandler):
            ...
            def post(self):
                try:
                    file_item = self.request.files.get('file', [None])[0]
                    if not file_item:
                        self.write({"ok": False, "headers": []})
                        return
                    
                    ext = os.path.splitext(file_item['filename'])[1] if file_item['filename'] else ".xlsx"
                    with tempfile.NamedTemporaryFile(delete=False, suffix=ext) as tmp:
                        tmp.write(file_item['body'])
                        tmp_path = tmp.name

                    raw_headers = get_headers(tmp_path)
                    ...
                    self.write({"ok": True, "headers": headers})
                except Exception as e:
                    self.write({"ok": False, "headers": [], "error": str(e)})

        class PythonCompareHandler(tornado.web.RequestHandler):
            def set_default_headers(self):
                self.set_header("Access-Control-Allow-Origin", "*")
                self.set_header("Access-Control-Allow-Headers", "*")
                self.set_header("Access-Control-Allow-Methods", "POST, GET, OPTIONS")
            ...
            def post(self):
                try:
                    file_old_item = self.request.files.get('file_old', [None])[0]
                    file_new_item = self.request.files.get('file_new', [None])[0]
                    file_ref_item = self.request.files.get('file_ref', [None])[0]

                    if not file_old_item or not file_new_item:
                        self.set_status(400)
                        self.write({"error": "يرجى اختيار ملف الموقع وملف المخزن"})
                        return

                    with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(file_old_item['filename'])[1]) as tmp_old:
                        tmp_old.write(file_old_item['body'])
                        tmp_old_path = tmp_old.name

                    with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(file_new_item['filename'])[1]) as tmp_new:
                        tmp_new.write(file_new_item['body'])
                        tmp_new_path = tmp_new.name

                    tmp_ref_path = ""
                    if file_ref_item:
                        with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(file_ref_item['filename'])[1]) as tmp_ref:
                            tmp_ref.write(file_ref_item['body'])
                            tmp_ref_path = tmp_ref.name

                    with tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx") as tmp_out:
                        output_path = tmp_out.name

                    key_old = self.get_argument("key_col_old", "")
                    key_new = self.get_argument("key_col_new", "")
                    comp_old = self.get_argument("comp_col_old", "")
                    comp_new = self.get_argument("comp_col_new", "")
                    mode = self.get_argument("mode", "compare")
                    new_col_name = self.get_argument("new_col_name", "المخزون الساحب")
                    ignore_punct = self.get_argument("ignore_punct", "true").lower() == "true"
                    try: sim_thresh = float(self.get_argument("similarity_threshold", "70"))
                    except: sim_thresh = 70.0
                    filter_keywords = self.get_argument("filter_keywords", "")

                    compare_excel_files(
                        tmp_old_path, tmp_new_path, output_path,
                        key_col_old=key_old, key_col_new=key_new,
                        comp_col_old=comp_old, comp_col_new=comp_new,
                        ignore_punct=ignore_punct, similarity_threshold=sim_thresh,
                        use_ai=False, ai_api_key="", mode=mode, new_col_name=new_col_name,
                        filter_keywords=filter_keywords, ref_filepath=tmp_ref_path
                    )
                    ...
                    self.set_header("Content-Disposition", 'attachment; filename="Tekno_Data_Report.xlsx"')
        ...
        # Dynamically append REST endpoints
        app.add_handlers(r".*", [
            (r"/api/python/headers", PythonHeadersHandler),
            (r"/api/python/compare", PythonCompareHandler),
        ])
except Exception as e:
    pass
